...
public class max_sum_rect {
    ...
    public static void load_test() {
        File fil = new File("judge/tests/max-sum-rect.txt");
    ...
    public static void judge() {
        ...
        for(int i = 0; i < num_test; ++i) {
            ...
            int answer = s.max_sum_rect(in_0[i]);
            if(answer != out[i]) {
                common.release_stdout();
                System.out.printf("%d / %d;", i+1, num_test);
                String outs = common.to_string(max_sum_rect.in_org_0[i]);
