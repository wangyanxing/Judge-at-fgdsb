...
public class coin_change_2 {
    public static int num_test = 303;
    ...
    public static void load_test() {
        File fil = new File("judge/tests/coin-change-2.txt");
    ...
    public static void judge() {
        ...
        for(int i = 0; i < num_test; ++i) {
            ...
            int answer = s.count_changes(in_0[i], in_1[i]);
            if(answer != out[i]) {
                common.release_stdout();
                System.out.printf("%d / %d;", i+1, num_test);
                String outs = common.to_string(coin_change_2.in_org_0[i]) + ", " + common.to_string(coin_change_2.in_org_1[i]);
