...
public class find_islands {
    ...
    public static void load_test() {
        File fil = new File("judge/tests/find-islands.txt");
    ...
    public static void judge() {
        ...
        for(int i = 0; i < num_test; ++i) {
            ...
            int answer = s.num_islands(in_0[i]);
            if(answer != out[i]) {
                common.release_stdout();
                System.out.printf("%d / %d;", i+1, num_test);
                String outs = common.to_string(find_islands.in_org_0[i]);
