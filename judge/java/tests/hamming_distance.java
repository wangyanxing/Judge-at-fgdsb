...
public class hamming_distance {
    ...
    public static void load_test() {
        File fil = new File("judge/tests/hamming-distance.txt");
    ...
    public static void judge() {
        ...
        for(int i = 0; i < num_test; ++i) {
            ...
            int answer = s.hamming(in_0[i], in_1[i]);
            if(answer != out[i]) {
                common.release_stdout();
                System.out.printf("%d / %d;", i+1, num_test);
                String outs = common.to_string(hamming_distance.in_org_0[i]) + ", " + common.to_string(hamming_distance.in_org_1[i]);
